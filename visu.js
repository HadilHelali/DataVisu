...
/***** Fonction  pour crier la matrice Scatterplot *****/
function scatterPlotMatrix() {
    ...
    cell.each(function ([i, j]) {
        ...
        if (i === 0 && j === 1) {
            path = currentPath; // Set the path only if i and j conditions are met
        }
    });
...
function parallelCoordinatesPlot() {
    ...
    const width = variables.length * 120; // Swap width and height
    const height = 928; // Swap width and height
    const marginTop = 20;
    ...
    const x = d3.scalePoint(variables, [marginLeft, width - marginRight]); // Swap x and y scales

    // the diagnosis will be the color
    color = d3
        .scaleOrdinal()
        .domain(data.map((d) => d.Diagnosis))
        .range(["#95d0fc", "#fb9a99"]);
    ...
    const svg = d3.create("svg")
        .attr("viewBox", [0, 0, width, height]) // Swap width and height
        .attr("width", width)
        .attr("height", height)
        .attr("style", "max-width: 100%; height: auto;");

    // Append Legend:
    const legendY = height - marginBottom - 20; // Adjust legend position
    const legend = svg.append("g")
        .attr("class", "legend")
        .attr("transform", `translate(${0},${legendY})`); // Adjust legend position
    ...
    const line = d3.line()
        ...
        .y(([ key, value ]) => y.get(key)(value)) // Swap x and y scales
        .x(([ key ]) => x(key)); // Swap x and y scales
    ...
    const axes = svg.append("g")
        ...
        .data(variables)
        .join("g")
        .attr("transform", (d) => `translate(${x(d)}, 0)`) // Swap x and y scales
        .each(function(d) {
            d3.select(this).call(d3.axisLeft(y.get(d))); // Swap x and y scales
        })
        .call((g) => g.append("text")
            .attr("x", 0)
            .attr("y", marginTop - 6) // Adjust text position
            .attr("text-anchor", "middle") // Adjust text alignment
...
function brushS(cell, circle, svg, { padding, size, x, y, variables, path }) {
    ...
    // Highlight the selected circles and update the shared selection.
    function brushed({ selection }, [i, j]) {
        ...
        parallelSvg.selectAll("path")
            .style("stroke", (d) =>
                selectedData.includes(d) ? color(d.Diagnosis) : deselectedColor)
    ...
    function brushended({ selection }) {
        ...
        // Update parallel coordinates plot with empty selection
        parallelSvg.selectAll("path")
            .style("stroke", (d) => (d && d.Diagnosis ? color(d.Diagnosis) : deselectedColor))
...
function brushP(svg, x, y, variables, color, path, marginTop, marginBottom, height,axes) {
    ...
    function brushed({selection}, key) {
        ...
        path.each(function (d) {
            ...
            d3.select(this).style("stroke", active ? color(d.Diagnosis) : deselectedColor);
            if (active) {
...
//Pour l'histogramme
function brushH(g, x, y, selectedCol, BinVal, KernelVal, parallelSvg, scatterSvg) {
    ...
    function brushed({selection}) {
        ...
        scatterSvg.selectAll("circle")
            .classed("hidden", (d) => !selectedData.includes(d));
    }
}
